...
data class ParkingSpot(
    ...
    val placeType: String = "street",         // "street" | "garage"
    val isDisabledSpot: Boolean = false,      // da li je mesto za invalide
    val zone: String = "green"                // "green" | "red" | "extra"
)
...
object ParkingRepository {
    ...
    suspend fun addParkingSpot(
        ...
        placeType: String = "street",
        isDisabledSpot: Boolean = false,
        zone: String = "green"
    ): Result<String> {
        return try {
            ...
            val dup = db.collection("parkings")
                ...
                .await()
            if (!dup.isEmpty) {
                Result.failure(IllegalStateException("A parking spot already exists at that address."))
            } else {
